...
export const TECHNICAL_CHECKLIST: ChecklistItem[] = [
  // ========== Conexão API UAZAPI / WatasAPI ==========
  {
    id: 'api-1',
    label: 'Configuração de credenciais e URL base da UAZAPI/WatasAPI',
    status: 'pending',
    category: 'Conexão API UAZAPI / WatasAPI',
    priority: 'Alta',
    description: 'Variáveis de ambiente (API key, URL base); config centralizada para todas as chamadas à API de WhatsApp.',
  },
  {
    id: 'api-2',
    label: 'Autenticação (token/API key) nas requisições à API',
    status: 'pending',
    category: 'Conexão API UAZAPI / WatasAPI',
    priority: 'Alta',
    description: 'Header ou query com token em toda requisição à UAZAPI/WatasAPI; renovação se necessário.',
  },
  {
    id: 'api-3',
    label: 'Endpoint de health/status da conexão com a API',
    status: 'pending',
    category: 'Conexão API UAZAPI / WatasAPI',
    priority: 'Média',
    description: 'Rota ou job que verifica se a API está acessível; exibir status no painel de instâncias.',
  },
  {
    id: 'api-4',
    label: 'Webhook URL configurado no painel UAZAPI (recebimento de mensagens)',
    status: 'pending',
    category: 'Conexão API UAZAPI / WatasAPI',
    priority: 'Alta',
    description: 'URL pública do webhook (ex.: /api/webhooks/uazapi) registrada na UAZAPI para receber mensagens e eventos.',
  },
  {
    id: 'api-5',
    label: 'Tratamento de erros e retry (timeout, 5xx, rate limit)',
    status: 'pending',
    category: 'Conexão API UAZAPI / WatasAPI',
    priority: 'Alta',
    description: 'Retry com backoff em falhas temporárias; mensagem clara na UI quando a API falhar.',
  },
  {
    id: 'api-6',
    label: 'Envio de mensagem de texto via API (POST send-message)',
    status: 'pending',
    category: 'Conexão API UAZAPI / WatasAPI',
    priority: 'Alta',
    description: 'Chamada à API de envio (instance + number + text); integração com Edge ou API interna.',
  },
  {
    id: 'api-7',
    label: 'Recebimento e parsing do payload do webhook (mensagens, status)',
    status: 'pending',
    category: 'Conexão API UAZAPI / WatasAPI',
    priority: 'Alta',
    description: 'Webhook valida e parseia body (mensagem inbound, status da instância, etc.); salva em messages e atualiza leads.',
  },
  {
    id: 'api-8',
    label: '(Opcional) Log de requisições/respostas para debug',
    status: 'pending',
    category: 'Conexão API UAZAPI / WatasAPI',
    priority: 'Baixa',
    description: 'Registro de request/response (sem dados sensíveis) para diagnóstico de falhas.',
  },

  // ========== Banco de Dados e Arquitetura ==========
  {
    id: 'db-1',
    label: 'Schema inicial Supabase (projeto e conexão)',
    status: 'pending',
    category: 'Banco de Dados e Arquitetura',
    priority: 'Alta',
    description: 'Projeto Supabase criado; URL e chave anon no .env; cliente configurado no app.',
  },
  {
    id: 'db-2',
    label: 'Tabela companies (multi-tenant)',
    status: 'pending',
    category: 'Banco de Dados e Arquitetura',
    priority: 'Alta',
    description: 'Tabela companies com id, name, settings; base do multi-tenant.',
  },
  {
    id: 'db-3',
    label: 'Tabela users (company_id, role_id, auth)',
    status: 'pending',
    category: 'Banco de Dados e Arquitetura',
    priority: 'Alta',
    description: 'Users vinculados a company_id e role_id; integração com Supabase Auth.',
  },
  {
    id: 'db-4',
    label: 'Tabela roles e role_permissions',
    status: 'pending',
    category: 'Banco de Dados e Arquitetura',
    priority: 'Alta',
    description: 'Roles por empresa; role_permissions para controle de acesso por tela/ação.',
  },
  {
    id: 'db-5',
    label: 'Tabela leads (company_id, seller_id, status, conversation_status)',
    status: 'pending',
    category: 'Banco de Dados e Arquitetura',
    priority: 'Alta',
    description: 'Leads com telefone, company_id, seller_id, status, conversation_status (fila, em_atendimento, fechada).',
  },
  {
    id: 'db-6',
    label: 'Tabela messages (lead_id, sender, content, direction, created_at)',
    status: 'pending',
    category: 'Banco de Dados e Arquitetura',
    priority: 'Alta',
    description: 'Mensagens com lead_id, content, direction (inbound/outbound), tipo (text, image, etc.).',
  },
  {
    id: 'db-7',
    label: 'Tabela instances (WhatsApp por company/seller)',
    status: 'pending',
    category: 'Banco de Dados e Arquitetura',
    priority: 'Alta',
    description: 'Instâncias UAZAPI por empresa; instance_id, status, seller_id, company_id.',
  },
  {
    id: 'db-8',
    label: 'Tabela sellers (vínculo user/company/instance)',
    status: 'pending',
    category: 'Banco de Dados e Arquitetura',
    priority: 'Alta',
    description: 'Sellers vinculam user a company; usado em leads.seller_id e instances.',
  },
  {
    id: 'db-9',
    label: 'Tabela appointments (agendamentos)',
    status: 'pending',
    category: 'Banco de Dados e Arquitetura',
    priority: 'Alta',
    description: 'Agendamentos com lead_id, company_id, scheduled_at, status.',
  },
  {
    id: 'db-10',
    label: 'Índices por company_id, lead_id, created_at (performance)',
    status: 'pending',
    category: 'Banco de Dados e Arquitetura',
    priority: 'Média',
    description: 'Índices nas tabelas principais para listagens e filtros rápidos.',
  },
  {
    id: 'db-11',
    label: 'RLS (Row Level Security) por company_id nas tabelas',
    status: 'pending',
    category: 'Banco de Dados e Arquitetura',
    priority: 'Alta',
    description: 'Políticas RLS garantindo que cada empresa só acesse seus dados.',
  },
  {
    id: 'db-12',
    label: 'Migrações versionadas (controle de alterações do schema)',
    status: 'pending',
    category: 'Banco de Dados e Arquitetura',
    priority: 'Média',
    description: 'Scripts SQL versionados (ex.: supabase/migrations) para evolução do banco.',
  },
  {
    id: 'db-13',
    label: '(Opcional) Tabelas message_templates, tags, lead_tags',
    status: 'pending',
    category: 'Banco de Dados e Arquitetura',
    priority: 'Média',
    description: 'Templates de mensagem e tags por empresa; lead_tags para etiquetar conversas.',
  },
